...
## 二、完整 Demo 流程與螢幕截圖解析（10 大完整步驟）

本章節完整記錄由「病患端發起諮詢與整理」到「醫護端相機掃描與調閱」之 10 大閉環步驟實測畫面。
...
### 步驟 9：醫護端調閱入口與相機掃描 QR Code（Clinician Portal）
...
* **系統介面亮點**：
  ...
  * 醒目提供 **「開啟相機掃描 QR Code」** 藍色功能按鈕，支援使用手機相機或筆電鏡頭對準病患 QR Code 進行離線自動秒級解碼。
  * 同步保留手動輸入「病患一次性分享碼」與「病患 session_id」之雙軌容錯機制。
* **螢幕截圖**：

![步驟 9：醫護端調閱入口與相機掃描 QR Code 畫面](images/demo/IMG_0006_compressed.jpg)
...
### 步驟 10：醫護端調閱與閱後即焚（Gate D: PASS 臨床摘要）

* **測試情境**：醫師掃描或貼上分享碼後，成功調閱病患看診前整理資料。
* **系統反應與渲染呈現**：
  * 右上角醒目顯示 **`D gate：PASS`** 綠色徽章，代表該筆資料已通過 Gate D 輸出安全過濾。
  * 分享碼狀態標明：**`已使用一次（不可重放）`**，精準落實「閱後即焚（Burn-After-Reading）」資安規範，防止二次調閱洩漏。
...
![步驟 10：醫護端調閱與閱後即焚臨床摘要畫面](images/demo/IMG_0007_compressed.jpg)

* **展示與報告亮點**：
  * 醫師能在 10 秒內完全掌握病患主訴、用藥歷程與就醫目的，大幅節省問診時間。
  * 閱後即焚與全自動 Audit Log 紀錄，達到醫院級資訊安全標準。
