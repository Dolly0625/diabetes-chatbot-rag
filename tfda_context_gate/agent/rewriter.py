...
# ── 查詢重寫器（繁中註解）──────────────────────────────────────────────────
# 職責：僅做「保義重寫」——將口語/檢索不友善的 current_query 改寫為檢索友善形式，不新增醫療事實。
# 關鍵：validate_meaning_preserving_rewrite 為窄式安全檢查；圖的唯一回環 QUERY_REWRITER→QUERY_EXPANSION 僅重寫後觸發。
# 上限：max_rewrites=1，由圖強制；重寫後僅更新 current_query，original_query 保持溯源基準。
...
def validate_meaning_preserving_rewrite(original_query: str, rewritten_query: str) -> None:
    ...
    【繁中註解｜保義重寫校驗】
    - 僅做窄式安全檢查，非語意裁判；由 graph.py 的 rewrite_node 在寫入 current_query 前呼叫。
    - 規則一：original_query 中的英數 token（正則 [A-Za-z0-9][A-Za-z0-9+\\-]*）不可在重寫後遺失（防藥名丟失）。
    - 規則二：若重寫新增「疼痛/紅腫/發燒/感染/休克/昏迷/停藥/增加劑量/減量」等未提供醫療事實，直接拋 ValueError。
    - 校驗失敗由外層 runtime_stage 映射為 AGENT_FAILURE；original_query 永不被覆蓋，僅 current_query 更新。
    """
