...
- 預設模型由 env 決定（OLLAMA_AGENT_MODEL > ROUTER_LLM_MODEL），透過 ChatOllama 原生介面呼叫，預設端點 http://localhost:11434。
...
OLLAMA_AGENT_MODEL = env_value("OLLAMA_AGENT_MODEL", env_value("ROUTER_LLM_MODEL", "") or "") or ""
...
def build_agent_ollama_llm() -> Any:
    ...
    model = env_value("OLLAMA_AGENT_MODEL", env_value("ROUTER_LLM_MODEL", "") or "") or OLLAMA_AGENT_MODEL or ""
    if not model:
        raise RuntimeError("OLLAMA_AGENT_MODEL or ROUTER_LLM_MODEL is required for Ollama agent; set it in .env or use deterministic fallback")
    return ChatOllama(
        model=model,
        base_url=endpoint,
        temperature=0,
