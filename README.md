...
* 當系統正則與意圖規則偵測到急症關鍵字（如：胸口劇痛、呼吸困難、嚴重低血糖冒冷汗、意識不清）時，立即中斷常規對話與 RAG 檢索，提供 119 與急診就醫引導。
...
> 說明：目前展示與原型測試環境採用本機 FastAPI 伺服器搭配 ngrok 外網穿透方式運行，尚未部署至 GCP（Google Cloud Platform）雲端主機；雲端容器化部署列為後續規劃。
