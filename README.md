...
* 前端相機掃描：內建 jsQR 解碼函式庫，醫師可使用相機掃描病患出示之 QR Code 分享碼。
* 單次存取機制：分享碼具備 15 分鐘時效，後端讀取後即標記為已使用，避免重複調閱。
* 審計日誌：調閱操作記錄於 audit_logs 資料庫。

### 6. 系統日誌與模組耗時追蹤（Audit Logs & Latency Tracking）
* 模組耗時記錄：追蹤日誌記錄 Gate A 路由、語意辨識、RAG 檢索、Gate B 審核、Gate C 生成、狀態流轉與 Gate D 輸出檢驗之個別處理耗時與總耗時。
* 識別碼去識別化：符合隱私規範，病患識別碼採雜湊處理以保護個資。

---

## 操作流程截圖說明 (Demo Walkthrough Report)

展示涵蓋病患端問卷整理與醫護端調閱流程，共收錄 11 個步驟之操作截圖：
* **圖文完整報告**：詳見 [DEMO_WALKTHROUGH_REPORT.md](docs/DEMO_WALKTHROUGH_REPORT.md)
* **流程步驟**：
  1. 急症關鍵字偵測與轉介提示
  2. 衛教知識檢索與生成（含免責聲明）
  3. 藥袋 QR Code 解析與藥名帶入
  4. 導引用戶至看診前對談室網頁
  5. 看診整理第 1 階段：用藥與過敏史
  6. 看診整理第 2 階段：慢性病與家族史
  7. 看診整理第 3 階段：症狀描述與就醫提問
  8. 摘要確認卡片（8 項欄位確認）
  9. 產生分享碼與 QR Code（15 分鐘時效）
  10. 醫護端相機掃描與代碼輸入介面
  11. 醫護端讀取摘要內容與權限失效
...
```text
[病患手機 LINE / 網頁] ──► [完成問卷生成 QR Code] ──► [醫師後台相機掃碼] ──► [讀取摘要並標記失效]
```

### 1. 醫護端入口連結
醫師使用電腦瀏覽器或手機打開以下網址：
* 雲端線上展示：`https://dsc-neural-annual-physics.trycloudflare.com/clinician`（Demo 模式預填 `doctor-demo` 授權身分）
* 本地開發測試：`http://localhost:8000/clinician`

### 2. 病患端產生分享碼（QR Code）
* **途徑 A（LINE 聊天室）**：病患在 LINE 輸入 `準備看診`，回答問卷（或傳送藥袋相片），完成後系統生成 15 分鐘時效之 QR Code 與代碼。
* **途徑 B（網頁對談室）**：病患打開 `http://localhost:8000/demo/previsit`，完成對話後產生 QR Code。

### 3. 診間調閱與單次存取機制
1. 醫師在醫護端點擊 **「開啟相機掃描 QR Code」**，對準病患手機上的 QR Code。
2. 前端透過 `jsQR` 解析字串，向後端 `/api/clinician/share/redeem` 發送兌換請求。
3. 後端核對 Token 有效期（15 分鐘），驗證成功後**將該 Token 標記為已使用**，防止重複讀取。
4. 醫護端顯示結構化病歷摘要（用藥歷程、過敏病史、主訴症狀、就醫提問），並記錄於 `audit_logs` 審計庫。
